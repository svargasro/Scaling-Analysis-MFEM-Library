#!/bin/bash
#SBATCH --job-name=miniapp      # Nombre del trabajo con ID de trabajo 
#SBATCH --output=results/slurm/out/miniapp_order_%A_%a.out  # Nombre del archivo de salida
#SBATCH --error=results/slurm/error/miniapp_order_%A_%a.err   # Nombre del archivo de error
#SBATCH --partition=AMDRyzen7PRO5750G                    # Aquí especificas la partición
#SBATCH --ntasks=1                                       # Una tarea por trabajo
#SBATCH --nodes=1                                        # Usar un nodo por tarea
#SBATCH --array=0-9                               # Ajusta esto al número total de ejecutables (3 en este ejemplo)
#SBATCH --exclusive                                      # Exclusivo acceso al nodo

MAX_ARR=9


# Definir los valores de ORDER correspondientes al índice del array

MAX_ORDER=$(($MAX_ARR + 1))  # Cálculo aritmético correcto

ORDERS=($(seq 1 $MAX_ORDER))  # Lista de valores de ORDER

# Asignar el valor de ORDER basado en el índice del array (%a)
