...
for orden in $ORDER; do
    ...
    for Nreps in $REPS; do
        ...
        if [ "$Nreps" -ne "$(echo "$REPS" | tail -n 1)" ]; then
	        resultado=$(mpirun -np $THREADS --oversubscribe ./${TARGET} -o $orden 2>&1 >/dev/null | tail -n 1) #Enviar stdout a /dev/null y el stderr a la variable
	        echo -n "$resultado," >> $timeOutput
        else
            resultado=$(mpirun -np $THREADS --oversubscribe ./${TARGET} -o $orden 2>&1 >/dev/null | tail -n 2) #Similar al anterior pero se capturan las últimas dos líneas en vez de solo una, para capturar el tamaño.
